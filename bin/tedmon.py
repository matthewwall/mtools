#!/usr/bin/env python
__version__ = '0.2.1'
"""Data collector/processor for TED5000
...
Collect data from a TED5000.  This script periodically queries a TED5000
gateway for the LiveData.xml file then saves the results to database or uploads
to any number of hosted services.
"""
__author__ = 'mwall'
__app__ = 'tedmon'
...
DB_DATABASE = 'ted'
DB_TABLE = 'ted'
DB_FILENAME = 'ted.db'        # filename for sqlite databases
...
class XMLPacket(object):
    ...
    def getchannels(self, packet):
        ...
        for label in packet.keys():
            if label.startswith('mtu') and label.endswith('_w'):
                channels.append(label[0:4])
        return channels
...
#        dbgmsg(xml)
        ...
        if elem is None:
            raise ReadError('no hostname element found')
        pkt['hostname'] = elem.text
        elem = root.find('Power')
        if elem is None:
            raise ReadError('no Power element found')
        for e in elem:
            item = e.tag.lower()
            for p in e:
                if p.tag == 'PowerNow':
                    pkt[item+'_w'] = float(p.text)
        elem = root.find('Voltage')
        if elem is None:
            raise ReadError('no Voltage element found')
        for e in elem:
            item = e.tag.lower()
            for p in e:
                if p.tag == 'VoltageNow':
                    pkt[item+'_v'] = float(p.text) / 10
        return pkt
    ...
    def printPacket(self, p):
        ...
        for x in range(1, 5):
            print ts + ": MTU%d Volts:          % 5dV" % (x, p['mtu%d_v' % x])
            print ts + ": MTU%d Power:          % 5dW" % (x, p['mtu%d_w' % x])
...
class TED5000Schema(BaseSchema):
    schema = [('time_created', 'INTEGER NOT NULL', '%d'), # unix epoch
              ('hostname', 'VARCHAR(32)', "'%s'"),        # hostname of device
              ('mtu1_v', 'REAL', '%.4f'),
              ('mtu1_w', 'REAL', '%.4f'),
              ('mtu2_v', 'REAL', '%.4f'),
              ('mtu2_w', 'REAL', '%.4f'),
...
class SocketClientCollector(BufferedDataCollector):
    ...
    def _read(self):
        ...
        hstr = '<hostname>%s</hostname></LiveData>' % self._host
        data = data.replace('</LiveData>', hstr)
        return [data]
        ...
        for p in packets:
            for x in self._getchannels(p):
                ...
                if not label in values:
                    values[label] = []
                values[label].append(self._getvalues(p['time_created'], p[x+'_w'], t))
...
class OpenEnergyMonitorProcessor(UploadProcessor):
    ...
    def _process_compiled(self, packets):
        for p in packets:
            ...
            data = []
            for idx, c in enumerate(PACKET_FORMAT.getchannels(p)):
                data.append('%s:%.2f' % (mklabel(devid, c), p[c+'_w']))
            if len(data):
...
if __name__ == '__main__':
    ...
    if not (options.print_out or options.mysql_out or options.sqlite_out or options.rrd_out or options.oem_out or options.pachube_out or options.smartenergygroups_out or options.thingspeak_out):
