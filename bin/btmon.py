...
class MQTTProcessor(BaseProcessor):
    def __init__(self, host, port, clientid, base_topic, qos, retain, will, user, passwd, tls, map, period):
        if not publish:
        ...
        infmsg('MQTT: mqtt:%s:%d?clientid=%s' % (self.host, self.port, self.clientid))
    ...
    def setup(self):
        if self.user == None and self.passwd != None:
            print 'MQTT Error: mqtt-user parameter must be provided if mqtt-passwd configured'
            sys.exit(1)
    ...
    def process_calculated(self, packets):
        ...
        if len(self._msgs):
            dbgmsg('MQTT: len=%d, msgs=%s' % (len(self._msgs), json.dumps(self._msgs)))
