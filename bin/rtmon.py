#!/usr/bin/python -u
__version__ = '0.2.1'
"""Data collector/processor for RadioThermostat devices
...
Collect data from a Radio Thermostat controller.  This script queries a Radio
Thermostat device then saves the results to database or uploads to any number
of hosted services.

Notes:
  Do not sample more often than every 10 seconds.
  Preferably sample only every 60 seconds since the USNAP is really slow.

This has been tested with the following:
  api_version fw_version  wlan_fw_version hardware
  113         1.04.84     v10.105576      CT50 V1.94 (filtrete branded device)

API reference: WiFi USNAP Module API v1.3 22mar2012
  which claims to apply to the following:
    system api: 113
    thermostat api: 100
    firmware: 1.04.82
    hardware:
...
The following API elements do not work:
  tstat/t_heat
  tstat/t_cool
  tstat/it_heat
  tstat/it_cool
...
/time
  ...
  tstat/ttemp
  {"t_heat":65.00,"t_cool":78.00}

  tstat/fmode
  {"fmode":0}

  tstat/datalog
 {"today":{"heat_runtime":{"hour":2,"minute":20},"cool_runtime":{"hour":0,"minute":0}},"yesterday":{"heat_runtime":{"hour":0,"minute":27},"cool_runtime":{"hour":0,"minute":0}}}

tstat returns only a subset:
{"temp":65.00,"tmode":1,"fmode":0,"override":0,"hold":0,"t_heat":65.00,"tstate":0,"fstate":0,"time":{"day":5,"hour":7,"minute":41},"t_type_post":0}

sys/services returns the following:
{"service_names":["com.rtcoa.tstat:1.0","devices.controller.tstat:1.0"],"httpd_handlers":{"/tstat":[1,1],"/sys/network":[1,1],"/sys/updater":[0,1],"/sys/filesystem":[0,1],"/sys/fs-image":[0,1],"/sys/firmware":[1,1],"/sys/fw-image":[0,1],"/sys/command":[0,1],"/sys/services":[1,0],"/sys/mode":[1,1],"/sys/name":[1,1],"/sys/watchdog":[1,1],"/sys/diag/log":[1,0],"/sys/diag/stats/live":[1,0],"/sys/diag/stats/history":[1,0],"/cloud":[1,1]}}
"""
__author__ = 'mwall'
__app__ = 'rtmon'
...
# a sample period of 60s and max upload period of 15m (900s), with contingency
# of 5m (300s) server/network downtime yields a buffer of 20
...
DB_DATABASE = 'rt'
DB_TABLE = 'rt'
DB_FILENAME = 'rt.db'         # filename for sqlite databases
...
def obfuscate_devid(sn):
    """obfuscate a device identifier - expose only the last 3 digits"""
...
def mkfn(path, label):
    ...
    label = label.replace(':','_')
    label = label.replace('/','_')
    return '%s/rt_%s.rrd' % (path, label)

def mktag(label, prefix):
    return '%s%s' % (prefix, label)


# packet definition for CT50
#   hw: V1.94
#   api_version: 113
#   fw_version: 1.04.84
#   wlan_fw_version: v10.105576
class CT50Packet(object):
    def __init__(self):
        self.TMODE = {-1: 'ERROR', 0: 'OFF', 1: 'HEAT', 2: 'COOL', 3: 'AUTO'}
        self.FMODE = {-1: 'ERROR', 0: 'AUTO', 1: 'AUTO/CIRCULATE', 2: 'ON'}
        self.TSTATE = {-1: 'ERROR', 0: 'OFF', 1: 'HEAT', 2: 'COOL'}
        self.FSTATE = {-1: 'ERROR', 0: 'OFF', 1: 'ON'}
        self.DE = {-1: 'ERROR', 0: 'DISABLED', 1: 'ENABLED'}

    def getchannels(self, packet):
        return ['temp','tmode','fmode','override','hold','tstate','fstate','t_heat','t_cool']

    def read(self, collector):
        data = dict()
        # FIXME: handle errors
        jsondata = collector.requestdata('/sys')
        data.update(json.loads(jsondata))
#        jsondata = collector.requestdata('/sys/name')
#        data.update(json.loads(jsondata))
#        jsondata = collector.requestdata('/tstat/model')
#        data.update(json.loads(jsondata))
        jsondata = collector.requestdata('/tstat')
        data.update(json.loads(jsondata))
        jsondata = collector.requestdata('/tstat/ttemp')
        data.update(json.loads(jsondata))
        data['uuid'] = unicodedata.normalize('NFKD', data['uuid']).encode('ascii','ignore')
        return [data]
...
#        print ts+': Name: %s' % p['name']
#        print ts+': Model: %s' % p['model']
        print ts + ': UUID: %s' % p['uuid']
        print ts + ': Current Temperature: %.2f' % p['temp']
        print ts + ': Heat Temperature: %.2f' % p['t_heat']
        print ts + ': Cool Temperature: %.2f' % p['t_cool']
        print ts + ': Thermostat Mode: %s' % self.TMODE[p['tmode']]
        print ts + ': HVAC State: %s' % self.TSTATE[p['tstate']]
        print ts + ': Fan Mode: %s' % self.FMODE[p['fmode']]
        print ts + ': Fan State: %s' % self.FSTATE[p['fstate']]
        print ts + ': Override: %s' % self.DE[p['override']]
        print ts + ': Hold: %s' % self.DE[p['hold']]
...
class CT50Schema(BaseSchema):
    schema = [('time_created', 'INTEGER NOT NULL', '%d'), # unix epoch
              ('uuid', 'VARCHAR(32)', "'%s'"),              # uuid of device
              ('temp', 'REAL', '%.4f'),
              ('t_heat', 'REAL', '%.4f'),
              ('t_cool', 'REAL', '%.4f'),
              ('tmode', 'INTEGER', '%d'),
    ...
    def __init__(self):
        self.name = 'CT50Schema'

    def gettablesql(self):
        return ', '.join(["`%s` %s" % (x[0], x[1]) for x in self.schema])

    # FIXME: use ?,?,? syntax instead of explicit formats
    def insertdata(self, cursor, table, packet):
        parts = []
        parts.append("insert into %s (" % table)
        parts.append(','.join([x[0] for x in self.schema]))
        parts.append(") values (")
        parts.append(','.join([x[2] % packet[x[0]] for x in self.schema]))
        parts.append(")")
        sql = ''.join(parts)
        dbgmsg('DB: %s' % sql)
        cursor.execute(sql)
...
class SocketClientCollector(BufferedDataCollector):
    ...
    def requestdata(self, path):
        url = 'http://%s:%d%s' % (self._host, self._port, path)
        infmsg('SOCKET: url: %s' % url)
        response = urllib2.urlopen(url, None, self._timeout)
        data = response.read()
...
class MySQLProcessor(DatabaseProcessor, MySQLClient):
    ...
    def setup(self):
        try:
            cfg = MySQLConfigurator(self.db_host, self.db_user,
                                    self.db_passwd, self.db_database,
                                    self.db_table, self.db_schema)
            cfg.configure()
        except Exception:
            pass
    ...
    def configure(self):
        try:
            ...
            sql = 'create table if not exists %s (%s)' % (
                self.db_fqtable, self.db_schema.gettablesql())
            cursor.execute(sql)
            cursor.close()
...
class PachubeProcessor(UploadProcessor):
    ...
    def _process_compiled(self, packets):
        streams = dict()
        for p in packets:
            devid = obfuscate_devid(p[PKT_ID])
            ts = mkts(p['time_created'])
            for idx, c in enumerate(PACKET_FORMAT.getchannels(p)):
                if p[c] != -1:
